"""
Solving Brock-Mirman optimal growth model
...
# import the bmvipi() function
from bmvipi import bmvipi
...
SSS, TM, SD, SM, SSD, SAC = acm(0,0.95,0.00712,7,1)
...
if findergk == 'y':
    match me:
        case 1:
            v,gk,gc,gi,gy,t = bmvipi(0,0,PAR, DSS, SSK, SSS, TM)
        ...
        case 2:
            v,gk,gc,gi,gy,t = bmvipi(1,0,PAR, DSS, SSK, SSS, TM)
            
            ergk_vi_dsc = SSK[[np.where(gk[:,0] > SSK)[0][-1]+1,\
                               np.where(gk[:,-1] < SSK)[0][0]-1]]/bk
        ...
        case 3:
            v,gk,gc,gi,gy,t = bmvipi(1,1,PAR, DSS, SSK, SSS, TM)
...
if solvemodel == 'y':
    ...
    plt.tight_layout()
    plt.savefig('growth_CDF_eff_plot1.jpg', dpi=800)
    plt.show()
    plt.close(fig1)
