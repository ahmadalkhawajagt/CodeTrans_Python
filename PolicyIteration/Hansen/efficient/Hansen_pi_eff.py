...
if findergk == 'y':
    match me:
        case 1:
            ...
            with open('ergk_rbcpi.npy', 'wb') as f:
                np.save(f, ergk_pi)
            
        case 2:
            v,gk,gh,gc,gi,gy,t = rbcvipi(1,0,PAR, DSS, SSK, SSS, TM)
            
            ergk_vi_dsc = SSK[[np.where(np.around(gk[:,0],4) >\
                                        np.around(SSK,4))[0][-1]+1,\
                               np.where(np.around(gk[:,-1],4) < \
                                        np.around(SSK,4))[0][0]-1]]/bk
            print('ergodic set of capital:', ergk_vi_dsc)
            with open('ergk_rbcvid.npy', 'wb') as f:
        ...
        case 3:
            ...
            with open('ergk_rbcvic.npy', 'wb') as f:
...
if findergk == 'y':
    fig, ax = plt.subplots()
    ax.plot(SSK, SSK, 'k:', label='45 degree line')
    ax.plot(SSK, gk[:,0], 'k-', label='lowest shock')
    ax.plot(SSK, gk[:,-1], 'k--', label='highest shock')

    plt.xlabel('$k$', fontsize=14)
    ax.set(title='Capital Policy Function')
    ax.grid()
    ax.legend()

    fig.savefig("Hansen_pi_eff_capital_policy.jpg", dpi=800)
    plt.show()
...
if solvemodel == 'y':
    match me:
        case 1:
            with open('ergk_rbcpi.npy', 'rb') as f:
                ergk_pi = np.load(f)
            ...
            save_shelf = shelve.open('./rbc_pi_ergk.pkl','n')
            for k in dir():
                try:
        ...
        case 2:
            with open('ergk_rbcvid.npy', 'rb') as f:
            ...
            save_shelf = shelve.open('./rbc_vi_dsc_ergk.pkl','n')
            for k in dir():
                try:
                    save_shelf[k] = globals()[k]
                except Exception:
        ...
        case 3:
            with open('ergk_rbcvic.npy', 'rb') as f:
            ...
            save_shelf = shelve.open('./rbc_vi_ctn_ergk.pkl','n')
            for k in dir():
                try:
                    save_shelf[k] = globals()[k]
                except Exception:
    ...
    plt.tight_layout()
    plt.savefig('Hansen_pi_eff_policy_value.jpg', dpi=800)
    plt.show()
    plt.close(fig1)
...
#--------------------------------------------------------------------------
# Simulate the RBC model (divisible labor supply)
#--------------------------------------------------------------------------
    ...
    for n in np.arange(0,N):
        ...
        hpd[:,[1]] = logsimi - np.linalg.solve(fm,logsimi)
        hpd[:,[2]] = logsimc - np.linalg.solve(fm,logsimc)
    ...
    print(np.concatenate(((statsim[0:6]*100)[:,np.newaxis], \
                          ((statsim[0:6]*100)/(statsim[0]*100))[:,np.newaxis],\
                          statsim[6:12][:,np.newaxis]),axis=1))
